...
import { assignRoles, getRoleFromCard, MIN_PLAYERS, MAX_PLAYERS, type Role } from '../../lib/game';
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (playerCount < MIN_PLAYERS || playerCount > MAX_PLAYERS) {
      return new Response(JSON.stringify({ error: `Need between ${MIN_PLAYERS} and ${MAX_PLAYERS} players to start` }), { status: 400 });
    }
    ...
    return new Response(JSON.stringify({ success: true, gamePhase: 'countdown' }), {
      status: 200,
